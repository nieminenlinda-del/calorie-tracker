...
A second import path, `src/health/importShortcutJson.ts`, reads iOS Shortcuts JSON (`schema: linda-health-shortcut`) from iCloud Drive. There is no backend POST: GitHub Pages is static, and IndexedDB lives on the phone. The Shortcut overwrites `linda-health-shortcut.json`; Ravinto upserts `daily_active_energy` by Helsinki date (Activity Summary preferred) and optional workout `health_samples`. See `docs/SHORTCUTS.md`.
