# Linda Health Sync — iOS Shortcuts → Ravinto

Daily active energy from Apple Health (Watch, Polar Flow, etc.) into Ravinto **without** Polar AccessLink, a native HealthKit app, or a backend.

Ravinto is a static GitHub Pages PWA. There is no server to POST into. The Shortcut writes a JSON file to **iCloud Drive**; Linda opens Ravinto and imports that file into the existing IndexedDB `linda-health` (same store as the Health `export.xml` import).
...
- `workouts` is optional. When present, they are stored as `health_samples`.
...
This matches Ravinto’s Health export logic (`dailyEnergyFromSummary`).
...
If **Save File** only offers the Shortcuts folder, save to `Shortcuts/Linda Health/linda-health-shortcut.json` instead and pick that same file in Ravinto. Create the subfolder in Files first.
...
Then each morning: open Ravinto → **Tuo Shortcuts JSON** → pick the overwritten file (iCloud may take a few seconds to sync).
